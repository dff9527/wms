...
| 識別碼 | 用途 | 格式範例 | 生成時機 |
|--------|------|---------|---------|
| **internal_sku** | 料件代碼 | `IC-001` | 料號主檔建立時 |
| **internal_lot_number** | 批次追溯 | `IC-001-240501-0001` | 每次收貨時 |
| **internal_barcode** | 掃描識別 | `240501-IC001-0001-W15` | 每次收貨時 |
| **vendor_lot_code** | 原廠追溯 | `TI2024W15A` | 解析供應商條碼 |

#### 6.1.2 批號生成邏輯完整代碼

```python
# backend/app/core/warehouse/receiving.py
...
class ReceivingService:
    ...
    def _generate_lot_number(self, sku: str, receive_date: datetime) -> str:
        """
        生成內部批號
        
        格式: {SKU}-{YYMMDD}-{流水號}
        範例: IC-001-240501-0001
        
        說明:
        - SKU: 料號代碼,方便人眼識別
        - YYMMDD: 收貨日期,用於 FIFO 排序
        - 流水號: 當日第幾批,從 0001 開始
        """
        date_str = receive_date.strftime('%y%m%d')
        
        # 查詢當日同料號已收幾批
        count = self.db.query(InventoryLot).filter(
            InventoryLot.internal_sku == sku,
            InventoryLot.receive_date >= receive_date.replace(hour=0, minute=0, second=0),
            InventoryLot.receive_date < receive_date.replace(hour=23, minute=59, second=59)
        ).count()
        
        sequence = f"{count + 1:04d}"
        
        return f"{sku}-{date_str}-{sequence}"
    
    def _generate_barcode(
        self, 
        sku: str, 
        lot_number: str,
        vendor_lot_code: str = None
    ) -> str:
        """
        生成內部條碼 (可掃描)
        
        格式: {YYMMDD}-{SKU簡碼}-{流水號}-{供應商批號後綴}
        範例: 240501-IC001-0001-W15
        
        說明:
        - YYMMDD: 快速識別收貨日期
        - SKU簡碼: 去除符號的料號 (IC-001 → IC001)
        - 流水號: 從批號提取
        - 供應商批號後綴: 原廠追溯用 (可選)
        """
        # 從批號提取日期和流水號
        # IC-001-240501-0001 → ['IC-001', '240501', '0001']
        parts = lot_number.split('-')
        date_part = parts[-2]
        seq_part = parts[-1]
        
        # SKU 簡碼 (去除符號)
        sku_short = sku.replace('-', '').replace('_', '')
        
        # 供應商批號後綴 (取最後幾碼)
        vendor_suffix = ""
        if vendor_lot_code:
            vendor_suffix = f"-{vendor_lot_code[-4:]}"
        
        return f"{date_part}-{sku_short}-{seq_part}{vendor_suffix}"
    ...
    def process_receipt(
        ...
        # Step 1: 解析條碼
        parser = BarcodeParser(self.db)
        parsed_data = parser.parse(scanned_barcode, vendor_id=...)
        
        if not parsed_data:
            raise ValueError("條碼解析失敗!")
        
        # Step 2: 驗證 PO
        po = self._verify_po(po_number, parsed_data['vendor_pn'])
        
        # Step 3: 生成內部批號
        receive_date = datetime.now()
        internal_sku = po.internal_sku
        
        internal_lot_number = self._generate_lot_number(
            sku=internal_sku,
            receive_date=receive_date
        )
        
        # Step 4: 生成內部條碼
        internal_barcode = self._generate_barcode(
            sku=internal_sku,
            lot_number=internal_lot_number,
        ...
        lot = InventoryLot(
            ...
            vendor_pn=parsed_data['vendor_pn'],
            vendor_lot_code=parsed_data.get('lot_code'),
            vendor_date_code=parsed_data.get('date_code'),
            original_barcode=scanned_barcode,  # 保留原始條碼
            
            quantity_on_hand=qty,
            receive_date=receive_date,
            lot_status='QC_HOLD',  # 待 IQC 檢驗
            
            raw_scan_data=parsed_data  # 供 Debug
        )
        ...
        label_url = printer.print_label({
            'internal_barcode': internal_barcode,
            'internal_lot_number': internal_lot_number,
            'description': po.item.description,
            'vendor_lot': parsed_data.get('lot_code'),
            'quantity': qty,
            'unit': 'PCS',
...
```
2024/05/01 早上 10:00 收貨:
- 料號: IC-001
- 生成批號: IC-001-240501-0001
- 生成條碼: 240501-IC001-0001-W15

2024/05/01 下午 14:00 收貨 (同料號):
- 料號: IC-001  
- 生成批號: IC-001-240501-0002  ← 流水號 +1
- 生成條碼: 240501-IC001-0002-W16
```
...
2024/05/01 收貨:
- 料號: IC-002  ← 不同料號
- 生成批號: IC-002-240501-0001  ← 流水號重新從 0001 開始
- 生成條碼: 240501-IC002-0001-A12
```
...
2024/05/15 收貨 (同料號):
- 料號: IC-001
- 生成批號: IC-001-240515-0001  ← 日期不同,流水號重新計數
- 生成條碼: 240515-IC001-0001-W20
```
...
def complete_iqc(
    ...
    PASS: 狀態改為 AVAILABLE,可以上架
    FAIL: 狀態改為 QUARANTINE,移至隔離區
    """
    lot = self.db.query(InventoryLot).get(lot_id)
    
    if not lot:
        raise ValueError(f"批次 {lot_id} 不存在")
    
    lot.iqc_result = result
    lot.iqc_date = datetime.now()
    ...
    if result == 'PASS':
        ...
        putaway = PutAwayEngine(self.db)
        suggested_location = putaway.suggest_location(lot)
        ...
        # 移至隔離區
        quarantine_zone = self._get_quarantine_location()
        lot.location_id = quarantine_zone.location_id
        ...
        return {
            'success': True,
            'status': 'QUARANTINE',
            'location': quarantine_zone.location_code
        }
```
...
此規格書提供給 Cursor AI 進行編碼。所有代碼範例可直接使用,資料庫 Schema 可直接執行。
