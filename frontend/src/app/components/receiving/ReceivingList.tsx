...
import { Package, Check, Clock, AlertCircle, Hourglass } from 'lucide-react';
...
export default function ReceivingList({ items, onViewDetails }: ReceivingListProps) {
  const getStatusBadge = (status: ReceivingItem['lotStatus']) => {
    ...
    const config = statusConfig[status] ?? statusConfig.PENDING_RECEIVE;
    return (
      <span
