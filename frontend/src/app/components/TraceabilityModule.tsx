...
// FIX: [fix_4] — Remove unused 'ArrowRight' import to resolve TS6133 error.
...
const DEMO_TRACE: TraceForwardResult = {
  barcode: '',
  type: 'internal_lot',
  supplier: {
...
export default function TraceabilityModule() {
  ...
  const handleSearch = () => {
    ...
    setTraceResult({
      ...
      type:
        searchBarcode.includes('TI20') || searchBarcode.includes('ST20') || searchBarcode.includes('ON24')
           ? 'vendor_lot'
           : searchBarcode.includes('-') && searchBarcode.length > 12
             ? 'internal_barcode'
             : 'internal_lot',
     });
   };
  ...
  const traceTimeline = [
     {
      ...
      color: 'bg-purple-500',
      details: [
         { label: '供應商', value: DEMO_TRACE.supplier.name },
         { label: '供應商批號 (vendor_lot_code)', value: DEMO_TRACE.supplier.vendorLotCode },
     ...
     },
     {
      ...
      color: 'bg-blue-500',
      details: [
         { label: 'PO 單號', value: DEMO_TRACE.supplier.poNumber },
         { label: 'IQC 檢驗', value: DEMO_TRACE.receiving.iqcResult },
         { label: '料號 (internal_sku)', value: DEMO_TRACE.receiving.internalSku },
         { label: '內部批號 (internal_lot_number)', value: DEMO_TRACE.receiving.internalLotNumber },
         { label: '內部條碼 (internal_barcode)', value: DEMO_TRACE.receiving.internalBarcode },
         { label: '上架儲位', value: DEMO_TRACE.inventory.location },
       ],
     },
     {
      step: 3,
     ...
     <div className="p-6 space-y-6">
       <div className="bg-white rounded-lg border border-slate-200 p-6">
         ...
         <div className="mt-4 flex flex-wrap gap-2 text-sm">
           <button
            type="button"
            onClick={() => setSearchBarcode('IC-001-240415-0001')}
           ...
           <button
            type="button"
            onClick={() => setSearchBarcode('240415-IC001-0001-W15')}
            className="px-3 py-1 bg-slate-100 text-slate-700 rounded hover:bg-slate-200"
           >
