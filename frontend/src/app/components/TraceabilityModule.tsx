  ...
  const traceTimeline = [
    ...
    },
    {
      ...
      details: [
        ...
        {
          label: '內部批號 (internal_lot_number)',
          value: traceResult?.receiving.internalLotNumber || '',
        },
        {
          label: '內部條碼 (internal_barcode)',
          value: traceResult?.receiving.internalBarcode || '',
        },
        ...
        <>
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-200 p-6">
            ...
            <p className="text-xs text-slate-600 mb-4">
              查詢鍵: <span className="font-mono">{traceResult.barcode}</span> · 類型推斷:{' '}
              {traceResult.type}
            </p>
            ...
            <div className="mt-4 bg-white rounded-lg p-4 border border-slate-200 flex justify-between flex-wrap gap-2">
              ...
              <p className="text-sm text-slate-700">
                ...
                <span className="font-semibold">
                  {traceResult.shipments.reduce((sum, s) => sum + s.qty, 0).toLocaleString()} PCS
                </span>
              </p>
            </div>
          </div>
                ...
                {traceTimeline.map((item, idx) => (
                  <div key={idx} className="relative flex gap-6">
                    <div className="relative z-10">
                      <div
                        className={`size-16 ${item.color} rounded-full flex items-center justify-center shadow-lg`}
                      >
