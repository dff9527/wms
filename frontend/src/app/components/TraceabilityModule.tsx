...
export default function TraceabilityModule() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      let response = await axios.get('/api/v1/trace/forward', { params: { query: searchBarcode } });
      
      if (response.data) {
        mapAndSetResult(response.data);
      } else {
        // Fallback to backward
        response = await axios.get('/api/v1/trace/backward', { params: { internal_barcode: searchBarcode } });
        if (response.data) {
          // Backward result structure might differ, adapt or show error
          // For now, assuming we want forward chain primarily. 
          // If backward only returns lot info, we might need to construct a partial view or just show the lot details.
          // The spec says "render the returned chain". Let's assume forward is primary for UI display.
          setError('Only backward trace available for this barcode.');
        }
      }
    } catch (err: any) {
