...
import { ScanBarcode, Package, CheckCircle2, Printer, Upload } from 'lucide-react';
import type { ReceivingListRow, ReceivingListRowStatus } from '../types/wms-inventory';

export type { ReceivingListRow, ReceivingListRowStatus };
...
export default function ReceivingModule() {
  ...
  const [receivingList] = useState<ReceivingListRow[]>([
    {
      id: 1,
      poNumber: 'PO-2024-0501',
      vendor: 'Texas Instruments',
      internalSku: 'IC-001',
      internalLotNumber: 'IC-001-240501-0001',
      internalBarcode: '240501-IC001-0001-W15',
    ...
    },
    {
      ...
      vendor: 'Texas Instruments',
      internalSku: 'IC-001',
      internalLotNumber: 'IC-001-240501-0002',
      internalBarcode: '240501-IC001-0002-W15',
      vendorLotCode: 'TI2024W15B',
      qty: 3000,
      status: 'iqc',
    },
    {
      id: 3,
      poNumber: 'PO-2024-0515',
      vendor: 'Texas Instruments',
      internalSku: 'IC-001',
      internalLotNumber: 'IC-001-240515-0001',
      internalBarcode: '240515-IC001-0001-W18',
      vendorLotCode: 'TI2024W18C',
      qty: 8000,
      status: 'completed',
    },
    {
      ...
      internalLotNumber: 'IC-STM358-240502-0001',
      internalBarcode: '240502-STM358-0001-A42',
    ...
    {
      id: 5,
      poNumber: 'PO-2024-0503',
      vendor: 'ON Semiconductor',
      internalSku: 'IC-ON2222',
      internalLotNumber: 'IC-ON2222-240503-0001',
      internalBarcode: '240503-ON2222-0001-X9',
  ...
  const handleReceive = () => {
    if (parsedData) {
      alert(
        `收貨成功!\n內部料號: ${parsedData.partNumber}\n供應商批號: ${parsedData.vendorLotCode}\n數量: ${parsedData.quantity} PCS\n（系統將產生內部批號與內部條碼並寫入 inventory_lots）`
      );
      setParsedData(null);
      setScannedBarcode('');
    }
  ...
  const getStatusBadge = (status: ReceivingListRowStatus) => {
    const statusConfig = {
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-2">
                支援格式: TI / ST / ON / NXP / Infineon 等主流供應商（解析結果為原廠欄位；內部批號／內部條碼於確認收貨後由系統產生）
              </p>
            </div>
          ...
          <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
            ...
            {parsedData ? (
              <div className="space-y-3">
                ...
                <div className="flex justify-between py-2 border-b border-slate-200">
                  <span className="text-sm text-slate-600">供應商料號／對照前料號</span>
                  <span className="text-sm font-mono font-medium text-slate-900">{parsedData.partNumber}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-slate-200">
                  <span className="text-sm text-slate-600">供應商批號</span>
                ...
                <div className="pt-4 flex gap-2">
                  ...
                  <button type="button" className="px-4 py-2 bg-slate-600 text-white rounded-lg hover:bg-slate-700 transition-colors flex items-center gap-2">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              料號 = internal_sku；內部批號 = internal_lot_number（FIFO）；內部條碼 = internal_barcode（掃描）；供應商批號 =
              vendor_lot_code（原廠追溯）
            </p>
          </div>
          <button type="button" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2 shrink-0">
          ...
          <table className="w-full min-w-[1100px]">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-left py-3 px-3 text-sm font-medium text-slate-600 whitespace-nowrap">採購單號</th>
                <th className="text-left py-3 px-3 text-sm font-medium text-slate-600 whitespace-nowrap">供應商</th>
                ...
                <th className="text-right py-3 px-3 text-sm font-medium text-slate-600 whitespace-nowrap">數量</th>
            ...
            <tbody>
              {receivingList.map((item) => (
                <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="py-3 px-3 text-sm font-mono text-slate-900 whitespace-nowrap">{item.poNumber}</td>
                  <td className="py-3 px-3 text-sm text-slate-700">{item.vendor}</td>
                  <td className="py-3 px-3 text-sm font-mono text-slate-900 whitespace-nowrap">{item.internalSku}</td>
                  <td className="py-3 px-3 text-sm font-mono text-slate-900 whitespace-nowrap">{item.internalLotNumber}</td>
                  <td className="py-3 px-3 text-sm font-mono text-slate-800 whitespace-nowrap">{item.internalBarcode}</td>
                  <td className="py-3 px-3 text-sm font-mono text-slate-700 whitespace-nowrap">{item.vendorLotCode}</td>
                  <td className="py-3 px-3 text-sm text-right text-slate-900 whitespace-nowrap">{item.qty.toLocaleString()} PCS</td>
