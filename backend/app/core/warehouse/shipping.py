...
class ShippingService:
    ...
    def confirm_shipment(
        self, so_number: str, shipper: str, shipping_notes: Optional[str] = None
    ) -> Dict:
        so = self._get_so(self.db, so_number)
        if not so:
            raise ValueError(f"Sales Order {so_number} not found")

        lines = self.db.query(SOLine).filter(SOLine.so_id == so.so_id).all()
        ...
        for line in lines:
            ...
            if picked < line.ordered_qty:
                raise ValueError(
                    f"Line {line.line_number} has insufficient picks ({picked}/{line.ordered_qty})"
                )
    ...
    def _get_shipment_details(self, so_number: str) -> List[Dict]:
        ...
        def _rkey(t):
            lot = t.lot
            return (
                lot.receive_date if lot and lot.receive_date else datetime.datetime.min
            )
            ...
            result.append(
                {
                    ...
                    "location": (
                        str(lot.location_id)
                        if lot and lot.location_id is not None
                        else None
                    ),
                    "receiveDate": (
    ...
    def generate_packing_list(self, so_number: str) -> Dict:
        ...
        packing_items = [
            {"sku": sku, "lots": items} for sku, items in sku_groups.items()
        ]

        return {"soNumber": so_number, "items": packing_items}
